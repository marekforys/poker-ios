...
class PokerGameViewModel: ObservableObject {
    ...
    @Published private(set) var winProbability: Double = 0.0
    ...
    func calculateWinProbability() {
        ...
        let knownCommunityCards = communityCards
        let dealerCards = dealer.hand.cards.filter { $0.isFaceUp }

        // Get remaining cards in deck
        let usedCards = playerCards + knownCommunityCards + dealerCards
        let remainingCards = deck.cards.filter { card in
            !usedCards.contains { $0.rank == card.rank && $0.suit == card.suit }
        }

        for _ in 0..<simulations {
            // Shuffle remaining cards
            var shuffledRemaining = remainingCards.shuffled()

            // Complete community cards if needed
            var simulationCommunityCards = knownCommunityCards
            while simulationCommunityCards.count < 5 && !shuffledRemaining.isEmpty {
                simulationCommunityCards.append(shuffledRemaining.removeFirst())
            }

            // Give dealer random cards if they don't have any
            var simulationDealerCards = dealerCards
            while simulationDealerCards.count < 2 && !shuffledRemaining.isEmpty {
                simulationDealerCards.append(shuffledRemaining.removeFirst())
            }

            // Evaluate hands
            let playerAllCards = playerCards + simulationCommunityCards
            let dealerAllCards = simulationDealerCards + simulationCommunityCards

            if playerAllCards.count >= 5 && dealerAllCards.count >= 5 {
                let playerBestHand = findBestHand(from: playerAllCards)
                let dealerBestHand = findBestHand(from: dealerAllCards)

                let playerEval = playerBestHand.evaluate()
                let dealerEval = dealerBestHand.evaluate()

                if playerEval.rank.rawValue > dealerEval.rank.rawValue {
                    wins += 1
                } else if playerEval.rank.rawValue == dealerEval.rank.rawValue {
    ...
    func dealFlop() {
        ...
        // Recalculate win probability after flop
        calculateWinProbability()
    ...
    func dealTurn() {
        ...
        if let turnCard = deck.deal() {
            ...
            // Recalculate win probability after turn
            calculateWinProbability()
    ...
    func dealRiver() {
        ...
        if let riverCard = deck.deal() {
            ...
            // Recalculate win probability after river
            calculateWinProbability()
